...
### Procedural Audio
Finally, instead of importing `.mp3` files, the native `AudioContext` API is utilised to synthesise sound effects directly in the browser. We create sine wave oscillators and manipulate their frequencies and durations on the fly so that the game generates retro "beeps" every time a laser is fired or a ship takes damage.
